let planet = {
    ...
    v: new Vector(2.929e4, Math.PI / 2),
    w: 0.005,
    width: 20 * 1.5e9,
    draw() {
        this.drawOrbit()
    ...
    drawVelocityDirection() {
        stroke(250, 50, 100, 90)
        let velocity = p5.Vector.normalize(this.v).mult(this.width * 2.5)
        line(this.r.x, this.r.y, this.r.x + velocity.x, this.r.y + velocity.y)
    },
    move(tof) {
        this.th = moveAngle(this.th, tof, this.T, this.e, -this.h.z / this.r.mag())
        // console.log(this.th)
        this.r = new Vector(rFromTh(this.th, this.e, this.h.mag(), star.mu), this.th)
    ...
    calculateOrbit() {
        ...
        if (isElliptic(this.e)) {
            this.T = sqrt(sq(TAU) * this.a * this.a * this.a / star.mu)
        }
    },
    drawOrbit() {
